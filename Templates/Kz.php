...
class Kz extends Regex
{
    ...
    protected $blockItems = array(
            ...
            4 => array(
                '/^Last modified(?>[\x20\t]*):(?>[\x20\t]*)(.+)$/im' => 'changed',
                '/^Domain created(?>[\x20\t]*):(?>[\x20\t]*)(.+)$/im' => 'created',
                '/^Domain status(?>[\x20\t]*):(?>[\x20\t]*)(.+)$/im' => 'status'
            ),
            5 => array(
                '/Current Registar:(?>[\x20\t]*)(.+)$/im' => 'registrar:name',
            ),
    );
    ...
    public function postProcess(&$WhoisParser)
    {
        ...
        if (isset($ResultSet->registrar->name)) {
            ...
            $registrar = strtolower(trim($ResultSet->registrar->name));
            $newConfig = $Config->get($registrar);
